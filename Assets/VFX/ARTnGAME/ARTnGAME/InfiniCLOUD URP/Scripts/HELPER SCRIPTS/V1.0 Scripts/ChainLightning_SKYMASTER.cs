...
//			particles = GetComponent<ParticleEmitter>().particles;
		...
		//v0.1
		public bool updateOnPositionchange = false;
		public Vector3 prev_pos;
		public int randomizeFactor = 30;
		public bool alignEndToStartLight = false;
		...
		{		
            ...
            //v0.1
            if (updateOnPositionchange && prev_pos == this_transform.position && Random.Range(0, randomizeFactor) != 1)
            {
				return;
			}
									...
									if (endLight){ 
										if(get_in==1 & target!=null){
											if (!SmoothLights)
											{
												...
												{
													endLight.transform.position = new Vector3(Saved_start_pos.x, target.transform.position.y, Saved_start_pos.z); //target.transform.position; //particles[particles.Length - 1].position; //v3.4.5b//v0.1
												}
												else
												{
