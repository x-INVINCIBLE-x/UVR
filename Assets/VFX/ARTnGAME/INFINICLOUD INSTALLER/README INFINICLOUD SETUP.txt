InfiniCLOUD URP - HDRP

--------------------------- InfiniCLOUD URP - HDRP INSTALLATION  --------------------------- 
IMPORTANT: 
The download contains two files, one for the URP and one for the HDRP pipeline.
Install the file needed depending on which pipeline is used. Then the installer folder can 
be removed.
-------------------------------------------------------------------------------------

The system is a shader based volumetric cloud generation framework, the clouds can 
also be used as ground fog and are best viewed from above or below the cloud bed 
or use for fast fly throughs. The system uses stacked planes to render the effect 
on the scene directly. Adding or removing layers can regulate the performance and 
visual quality of the system.

InfiniCLOUD is a sub module of Sky Master ULTIMATE (Included in all  Sky Master ULTIMATE pipeline versions).
Below are the Sky Master ULTIMATE tutorials for reference of what other cloud
systems are possible with the complete Sky Master weather suit and InfiniCLOUD used
with the time of day module, for reference.

Note that it is possible to upgrade from InfiniCLOUD to Sky Master ULTIMATE at a
reduced upgrade price.

Sky Master ULTIMATE Standard Pipeline tutorial videos:
https://www.youtube.com/watch?list=PLJQvoQM6t9Ge2ehO4N1kNq3jvHmVst_el&v=XXCMXiuM9VA

Sky Master ULTIMATE HDRP-URP Tutorial videos:
https://www.youtube.com/watch?list=PLJQvoQM6t9Gead80gg5MSyN-gsEzmO0LW&v=w6useVWMeMM

InfiniCLOUD v1.7.0 New Inspector with direct clouds and Vortex controls
https://www.youtube.com/watch?v=qk8KBaGV1bk
https://www.youtube.com/watch?v=LImGcuNXQcY

IMPORTANT: 
Note that the Custom Inspector controls correspond to the "Multi Quad C Clouds" group enabled with the checkbox
that correspond to the GameObject referenced in the "Side C Clouds" slot,
and to the "Cloud Sides C Mat" Material, thus will apply only to that group.

The other cloud groups and extra parameters like coloration of the cloud are in the advanced settings
ssection of the script.

To assign a different material to the clouds, copy the Material referecned in the "Cloud Sides C Mat" slot
and assign the copy to the slot. Then apply this material to all the planes or quads under the Gameobject referenced
in the "Side C Clouds" slot. Then all script parameters will apply to this material.
